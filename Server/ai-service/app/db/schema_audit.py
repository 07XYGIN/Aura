        ...
        if isinstance(constraint, PrimaryKeyConstraint):
            ...
            result["foreign"].add(
                (
                    ...
                    str(constraint.initially or "IMMEDIATE").upper() == "DEFERRED",
                )
            )
    return result
...
def audit_schema(database_url: str = SYNC_DATABASE_URL, *, require_framework: bool = True) -> list[str]:
    ...
    with psycopg.connect(database_url, row_factory=dict_row) as conn:
        ...
        errors: list[str] = []
        for table_name in sorted((FRAMEWORK_TABLES - database_tables) if require_framework else set()):
            errors.append(f"数据库缺少框架管理表：{table_name}")
        if require_framework and "alembic_version" in database_tables:
            expected_heads = migration_heads()
            actual_heads = {row["version_num"] for row in conn.execute("SELECT version_num FROM alembic_version")}
            if actual_heads != expected_heads:
                errors.append(f"数据库迁移未对齐：expected={sorted(expected_heads)}, actual={sorted(actual_heads)}；执行 uv run alembic upgrade head")
        ...
        for table_name, table in Base.metadata.tables.items():
            ...
            if table_name in database_tables:
                errors.extend(check_constraint_definitions(conn, table))
...
def check_constraint_definitions(conn, table) -> list[str]:
    """Compare PostgreSQL-normalized CHECK expressions using empty temporary tables."""
    checks = [c for c in table.constraints if isinstance(c, CheckConstraint)]
    if not checks:
        return []
    temp_name = f"aura_audit_{table.name}"
    definitions = [sql.SQL("LIKE {}.{}").format(sql.Identifier("public"), sql.Identifier(table.name))]
    definitions.extend(
        sql.SQL("CONSTRAINT {} CHECK ({})").format(sql.Identifier(c.name), sql.SQL(str(c.sqltext)))
        for c in checks
    )
    try:
        with conn.transaction():
            conn.execute(sql.SQL("CREATE TEMP TABLE {} ({}) ON COMMIT DROP").format(
                sql.Identifier(temp_name), sql.SQL(", ").join(definitions)))
            rows = conn.execute("""
                SELECT conname, pg_get_expr(conbin, conrelid) AS expression, convalidated,
                       conrelid = to_regclass(%s) AS expected
                FROM pg_constraint WHERE contype = 'c'
                  AND conrelid IN (to_regclass(%s), to_regclass(%s))
            """, (f"pg_temp.{temp_name}", f"pg_temp.{temp_name}", f"public.{table.name}")).fetchall()
            expected = {r["conname"]: r["expression"] for r in rows if r["expected"]}
            actual = {r["conname"]: r["expression"] for r in rows if not r["expected"]}
            errors = [f"检查约束内容不一致：{table.name}.{name}" for name, expr in expected.items() if actual.get(name) != expr]
            errors.extend(f"检查约束尚未验证：{table.name}.{r['conname']}" for r in rows if not r["expected"] and not r["convalidated"])
            conn.execute(sql.SQL("DROP TABLE pg_temp.{}").format(sql.Identifier(temp_name)))
            return errors
    except psycopg.Error as exc:
        return [f"无法核对检查约束：{table.name} ({exc.sqlstate})"]


def assert_schema_ready() -> None:
    errors = audit_schema()
    if errors:
        raise RuntimeError("数据库结构不匹配，服务启动已停止。先执行迁移及 tools/check_db_schema.py：\n" + "\n".join(errors))
